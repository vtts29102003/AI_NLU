...
//Nếu số lượng gạch không đúng chỗ thì  sẽ là 1, còn nếu đúng chỗ sẽ là 0, và cuối cùng đưa ra tổng h1 là số lượng gạch không đúng chỗ 
//số khoảng cách từ source tới goal nó cần đi bao nhiêu ô (không được đi chéo) và cuối cùng đưa ra tổng h2 là số lượng bước đi để cho tất cả mọi cái gạch đúng chỗ.
...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int row = 0; row < currentState.getRow(); row++) {
	        for (int col = 0; col < currentState.getColumn(); col++) {
	            int tile = currentState.getTile(row, col);
	            if (tile != 0) {
	                // Count the number of misplaced tiles
	                if (tile != (row * currentState.getColumn() + col + 1)) {
	                    output++;
	                }
	            }
	        }
	...
	public int computeH2(Node currentState) {
	    ...
	    for (int row = 0; row < currentState.getRow(); row++) {
	        for (int col = 0; col < currentState.getColumn(); col++) {
	            int tile = currentState.getTile(row, col);
	            if (tile != 0) {
	                // Calculate the Manhattan distance between the current tile and its target position
	                int[] target = currentState.getLocation(tile);
	                result += PuzzleUtils.manhattanDistance(whiteTile, target);
	            }
	        }
	    }
		return result;
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
	        ...
	        if (row < currentState.getRow()) {// Tile stands inside the map
	            int tmp = currentState.getTile(row, col);
	            result.updateTile(row, col, 0);
	            result.updateTile(whiteTile[0], whiteTile[1], tmp);
	            result.setH(computeH2(result));
